...
export function parseVmlElement(elem: Element, parser: DocumentParser): VmlElement {
	...
	for (const at of xml.attrs(elem)) {
		switch(at.localName) {
			case "style": 
				result.cssStyleText = formatCssRules(parseCssRules(at.value));
				break;

			case "coordorigin":
				result.coordOrigin = parsePoint(at.value);
				break;

			case "coordsize":
				result.coordSize = parsePoint(at.value);
				break;

			case "spt":
				result.shapeType = at.value;
				break;

			case "type":
				result.shapeType ??= at.value?.replace(/^#_x0000_t/i, "");
				break;
	...
	for (const el of xml.elements(elem)) {
		switch (el.localName) {
			...
			case "textpath":
				result.textPath = {
					text: xml.attr(el, "string") ?? "",
					style: parseCssRules(xml.attr(el, "style") ?? "")
				};
				break;
...
function parseStroke(el: Element): Record<string, string> {
	return {
		...
		'stroke-width': xml.lengthAttr(el, "weight", LengthUsage.Emu) ?? '1pt'
	};
}
