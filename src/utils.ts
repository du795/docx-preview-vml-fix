...
export function parseCssRules(text: string): Record<string, string> {
	const result: Record<string, string> = {}; // 内联样式键值映射

	for (const rule of text.split(';')) {
		const [rawKey, ...rawValues] = rule.split(':'); // 原始样式键值片段
		const key = rawKey?.trim(); // 样式键名
		const value = rawValues.join(':').trim(); // 样式键值
...
export function formatCssRules(style: Record<string, string>): string {
	return Object.entries(style).map(([key, value]) => `${key}: ${value}`).join(';'); // 样式对象序列化字符串
}
