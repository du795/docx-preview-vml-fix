...
// 支持优先解析 Word 2010 绘图命名空间，避免直接回退到兼容性较弱的 Fallback 分支
...
export class DocumentParser {
	...
	parseGraphic(elem: Element): OpenXmlElement {
		var graphicData = xml.element(elem, "graphicData"); // 绘图数据节点
		var graphicChildren = graphicData ? this.parseGraphicChildren(graphicData) : []; // 绘图子元素列表

		if (graphicChildren.length === 0)
			return null;

		return graphicChildren.length === 1 ? graphicChildren[0] : this.createDrawingContainer(graphicChildren);
	}

	// 解析 DrawingML 图形子节点，递归收集图片与形状
	parseGraphicChildren(node: Element): OpenXmlElement[] {
		var result: OpenXmlElement[] = []; // 当前层级可渲染的图形节点

		for (let child of xml.elements(node)) {
			child = this.checkAlternateContent(child); // AlternateContent 优先选择已声明支持的分支
			...
			switch (child.localName) {
				case "pic":
					result.push(this.parsePicture(child));
					break;

				case "chart":
					result.push(this.parseChartReference(child));
					break;

				case "wsp":
					result.push(this.parseWordProcessingShape(child));
					break;

				case "wpg":
				case "wpc":
					var nestedChildren = this.parseGraphicChildren(child); // 递归解析分组或画布内的图形
					var container = this.createDrawingContainer(nestedChildren); // 统一包装为可定位容器
				...
				default:
					result.push(...this.parseGraphicChildren(child));
					break;
			}
		}
	...
	// 创建绘图容器，统一承载多个 DrawingML 图形元素
	createDrawingContainer(children: OpenXmlElement[]): OpenXmlElement {
		if (!children?.length)
			return null;
	...
	// 解析 WordprocessingShape 形状节点，覆盖常见几何图形、描边、填充与文本框内容
	parseWordProcessingShape(node: Element): IDomShape {
		var result: IDomShape = {
			type: DomType.Shape,
			children: [],
			cssStyle: {
				position: "absolute",
		...
		for (const child of xml.elements(node)) {
			switch (child.localName) {
				case "spPr":
					this.parseWordProcessingShapeProperties(child, result);
					break;

				case "bodyPr":
					this.parseWordProcessingShapeBodyProperties(child, result);
					break;

				case "txbx":
					var textBoxContent = xml.element(child, "txbxContent"); // 形状文本框内容节点

					if (textBoxContent)
						result.children.push(...this.parseBodyElements(textBoxContent));
					break;
			}
		}
	...
	// 解析形状属性，包括几何类型、填充、边框与内部变换
	parseWordProcessingShapeProperties(node: Element, shape: IDomShape) {
		var xfrm = xml.element(node, "xfrm"); // 形状内部变换节点
		var line = xml.element(node, "ln"); // 形状边框节点
		var prstGeom = xml.element(node, "prstGeom"); // 预设几何图形节点
		var fillColor = this.parseDrawingFill(node); // 形状填充颜色
		var strokeColor = this.parseDrawingLineColor(line); // 形状描边颜色
		var strokeWidth = line ? xml.lengthAttr(line, "w", LengthUsage.Emu) : null; // 形状描边宽度

		if (prstGeom)
			shape.geometry = xml.attr(prstGeom, "prst") ?? shape.geometry;
		...
		var transforms: string[] = []; // 形状的旋转与翻转样式集合
		var rotation = xml.intAttr(xfrm, "rot", 0) / 60000; // 旋转角度
		var flipHorizontal = xml.boolAttr(xfrm, "flipH", false); // 是否水平翻转
		var flipVertical = xml.boolAttr(xfrm, "flipV", false); // 是否垂直翻转
		...
		if (transforms.length > 0) {
			shape.cssStyle["transform"] = transforms.join(" ");
			shape.cssStyle["transform-origin"] = "center center";
		}

		for (const child of xml.elements(xfrm)) {
			switch (child.localName) {
				case "off":
					shape.cssStyle["left"] = xml.lengthAttr(child, "x", LengthUsage.Emu);
					shape.cssStyle["top"] = xml.lengthAttr(child, "y", LengthUsage.Emu);
					break;

				case "ext":
					shape.cssStyle["width"] = xml.lengthAttr(child, "cx", LengthUsage.Emu);
					shape.cssStyle["height"] = xml.lengthAttr(child, "cy", LengthUsage.Emu);
					break;
			}
		}
	}

	// 解析文本框内边距与垂直对齐，保证形状中的文本可读
	parseWordProcessingShapeBodyProperties(node: Element, shape: IDomShape) {
		var anchor = xml.attr(node, "anchor"); // 文本框垂直对齐方式

		shape.textPadding = {
			left: xml.lengthAttr(node, "lIns", LengthUsage.Emu) ?? shape.textPadding.left,
			top: xml.lengthAttr(node, "tIns", LengthUsage.Emu) ?? shape.textPadding.top,
			right: xml.lengthAttr(node, "rIns", LengthUsage.Emu) ?? shape.textPadding.right,
			bottom: xml.lengthAttr(node, "bIns", LengthUsage.Emu) ?? shape.textPadding.bottom,
		};
		...
		switch (anchor) {
			...
			case "b":
				shape.textVerticalAlign = "flex-end";
				break;

			default:
				shape.textVerticalAlign = "flex-start";
				break;
		}
	}

	// 解析 DrawingML 填充颜色，当前优先支持无填充与纯色填充
	parseDrawingFill(node: Element): string {
		if (xml.element(node, "noFill"))
			return "none";

		return this.parseDrawingColorNode(xml.element(node, "solidFill"));
	}

	// 解析 DrawingML 描边颜色，当前优先支持无描边与纯色描边
	parseDrawingLineColor(node: Element): string {
		if (!node)
			return null;
		...
		return this.parseDrawingColorNode(xml.element(node, "solidFill")) ?? "black";
	}

	// 解析 DrawingML 颜色节点，兼容 srgb、scheme 与预设颜色
	parseDrawingColorNode(node: Element): string {
		if (!node)
			return null;

		var rgbColor = xml.element(node, "srgbClr"); // RGB 颜色节点
		var schemeColor = xml.element(node, "schemeClr"); // 主题颜色节点
		var presetColor = xml.element(node, "prstClr"); // 预设颜色节点

		if (rgbColor)
			return `#${xml.attr(rgbColor, "val")}`;

		if (schemeColor)
			return `var(--docx-${xml.attr(schemeColor, "val")}-color)`;

		if (presetColor)
			return xml.attr(presetColor, "val");
	...
	// 解析图表引用节点，后续在渲染阶段按关系 ID 加载实际 chart 部件
	parseChartReference(elem: Element): IDomChart {
		return {
			type: DomType.Chart,
			id: xml.attr(elem, "id"),
			cssStyle: {
